...
# -------------------------------------------------------------------------------------------------------------------
# COPIED FROM: https://raw.githubusercontent.com/mohsenfayyaz/DecompX/main/src/decompx_utils.py
import torch
from dataclasses import dataclass
...
# COPIED FROM: https://github.com/mohsenfayyaz/DecompX/blob/main/DecompX_Colab_Demo.ipynb
CONFIGS = {
    "DecompX":
        DecompXConfig(
            include_biases=True,
...
class BigBirdSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # NOTE: decompx returns attention_probs regardless of the flag output_attentions
...
#NOTE: This attention is not used when computing attributions, because we only do so for datasets
#      with shorter sequences, like SST and Movies-128. Datasets with longer sequences are too slow
#      or OOM when computing decompx / alti, even using a 48GB GPU.
...
class BigBirdSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor,
                decompx_ready=False) -> torch.Tensor: # added by Fayyaz / Modarressi
...
class BigBirdOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor, decompx_ready: Optional[bool] = False) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        # hidden_states = self.LayerNorm(hidden_states + input_tensor)
        # return hidden_states
...
class BigBirdLayer(nn.Module):
    ...
    def forward(
        ...
        if decompx_ready:
            ...
            if output_attentions:
                return (layer_output, attention_probs,) + (new_outputs,)
            else:
...
class BigBirdClassificationHead(nn.Module):
    ...
    def forward(self, features, decompx_ready=False, **kwargs):
        ...
        # NOTE - CHANGED: Replace this by tanh, so that it is similar to calling BigBirdModel and using the pooled output to classify
        #post_act = ACT2FN[self.config.hidden_act](pre_act)
